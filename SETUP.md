...
1. В Google Таблице создайте листы:
   ...
   - **"Аналитика продавца"** - для воронки продаж
...
#### 4.4. Триггер для воронки продаж

1. Добавьте еще один триггер:
   - **Выполнять функцию:** `runSalesFunnelSync`
   - **Источник события:** По времени
   - **Тип события:** Ежедневно
   - **Время:** Выберите удобное время (например, 03:00 - 04:00 ночи, после других отчетов)
3. Сохраните триггер

#### 4.5. Триггер для аналитики рекламных кампаний

1. Добавьте еще один триггер:
   - **Выполнять функцию:** `runAdsAnalyticsSync`
   - **Источник события:** По времени
   - **Тип события:** Ежедневно
   - **Время:** Выберите удобное время (например, 03:30 - 04:30 ночи, после воронки продаж)
3. Сохраните триггер

#### 4.6. Триггер для истории рекламных расходов

1. Добавьте еще один триггер:
   - **Выполнять функцию:** `runAdsCostsSync`
   - **Источник события:** По времени
   - **Тип события:** Ежедневно
   - **Время:** Выберите удобное время (например, 04:00 - 05:00 ночи, после аналитики РК)
3. Сохраните триггер
...
#### 5.4. Тест воронки продаж

1. В редакторе Apps Script выберите функцию `manualSalesFunnelSync`
2. Нажмите **Выполнить** (Run)
3. Проверьте логи выполнения (генерация отчета может занять до 10 минут)
4. Проверьте данные в Google Таблице

#### 5.5. Тест аналитики рекламных кампаний
...
#### 5.6. Тест истории рекламных расходов
...
- `reports/salesFunnel.gs` - Модуль воронки продаж (аналитика продавца)
...
2. **Проверка дубликатов:** 
   ...
   - Для воронки продаж: скрипт проверяет, не загружены ли уже данные за предыдущий день
...
7. **Воронка продаж:**
   - Используется endpoint `/api/v2/nm-report/downloads` (трехэтапный процесс: создание отчета, проверка статуса, скачивание ZIP с CSV)
   - Данные дозаписываются ежедневно за предыдущий день
   - Требуется токен для категории "Аналитика" (для подписки Джем)
   - Максимум 20 отчетов в сутки
   - Отчет хранится 48 часов
   - Лимит API: 3 запроса в минуту с интервалом 20 секунд

8. **Аналитика рекламных кампаний:**
...
Данные выгружаются в следующем порядке:
Дата, Артикул WB, Артикул продавца, Наименование, Предмет, Бренд, Переходы в карточку, Положили в корзину, Заказы, Заказы на сумму, Выкупы, Выкупы на сумму
